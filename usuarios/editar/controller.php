...
if ($_proceso == "updUsuario"){
	$params = new ParamsSQL();
	$params->setString("nombre",$_POST['nombre']);
	$params->setString("usuario",$_POST['usuario']);
	$params->setString("clave",$_POST['clave']);
	$params->setCondition("cod = ".$_POST['cod']);
	
	$ctrl = new TableSet();	
	$res = $ctrl->actualizarArg ($params, "usuarios");
	echo $res;
}
...
if ($_proceso == "getUsuario"){
	$ctrl = new TableSet();
	$cod = $_POST['cod'];
	
	$res = $ctrl->getJsonQuery ( "select * from usuarios where cod = ".$cod);
	echo $res;
}
