<body class="hold-transition skin-purple-light sidebar-mini">
	...
	<?php 
// PRE PHP _ by type
$_SESSION['load_file'] = "atx_generic_sidebar";

$mods = $_SESSION['mod_info'];
for ($i = 0; $i < count($mods); $i ++)
    if (isset($mods[$i]->path))
        if (isset($mods[$i]->loadPHP))
            include ($_SESSION['dircore'] . "/modules/" . $mods[$i]->path . "/" . $mods[$i]->loadPHP);
?>
  ...
  <!-- Left side column. contains the logo and sidebar -->
		...
		<footer class="main-footer">
			...
			<strong>Desarrollado por<i class="fa fa-love"></i><a
				href="https://www.kaizerdev.com"> Jorge Tordoya</a></strong>
		</footer>
...
for ($i = 0; $i < count($mods); $i ++)
    if (isset($mods[$i]->path))
        if (isset($mods[$i]->postPHP))
            include ($_SESSION['dircore'] . "/modules/" . $mods[$i]->path . "/" . $mods[$i]->postPHP);
?>
