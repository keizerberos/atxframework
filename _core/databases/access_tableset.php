...
function odbc_fetch_array_ok($result, $rownumber=null) {
	$array = array();
	...
	for ($i = 1; $i <= $cols; $i++) {
		...
		//$array[odbc_field_name($result, $i)] = utf8_encode ($result_array[$i - 1]);
	}
	return $array;
}
...
class ACCESS {
	var $con;
	var $nombreTabla;
	public function ACCESS($opt) {
		$this->con = odbc_connect("Driver={Microsoft Access Driver (*.mdb)};Dbq=".$opt["server"]."/".$opt["db"], $opt["user"], $opt["pass"]);
	...
	public function getArrayQuery($query,$de,$count,$sort) {
		...
		if (is_null($de))
			$resultado = odbc_exec ( $this->con, $query ) or die ( "Error " . odbc_exec ( $this->con ) ); 
		else
			$resultado = odbc_exec ( $this->con , $query . " limit $de,$count") or die ( "Error " . odbc_exec ( $this->con ) ); 
		...
		while ( $fila = odbc_fetch_array_ok ( $resultado, NULL ) ) {					
			array_push($js,array_map("utf8_encode", $fila ));
		}
